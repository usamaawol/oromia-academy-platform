//#region node_modules/.nitro/vite/services/ssr/assets/ai-import.server-CBI1n-Nj.js
var SYSTEM_PROMPT = `You are a question extraction assistant for Oromia Academy, an educational platform that uses Afaan Oromoo and English.
...
`;
function validateExtracted(q, idx) {
	const warnings = [...q.warnings ?? []];
	if (!q.questionOm?.trim() && !q.questionEn?.trim()) warnings.push("Question text is empty");
	if (q.type === "mcq") {
		if (!q.options || q.options.length < 2) warnings.push(`Only ${q.options?.length ?? 0} options found — expected at least 2`);
		if (!q.correctAnswer) warnings.push("Correct answer is missing");
		else if (!(q.options ?? []).map((o) => o.key).includes(q.correctAnswer)) warnings.push(`Correct answer "${q.correctAnswer}" does not match any option key`);
	...
	if (q.type === "truefalse") {
		if (!q.correctAnswer || !["true", "false"].includes(q.correctAnswer.toLowerCase())) warnings.push("Correct answer must be 'true' or 'false'");
	}
	const valid = warnings.filter((w) => w.includes("missing") || w.includes("empty") || w.includes("does not match")).length === 0;
	return {
		...q,
...
function splitIntoBlocks(text) {
	const blocks = text.split(/(?=(?:Question\s*\d+\s*:|Q\s*\d+\s*[:.]\s*|\d+[.)]\s+))/i).map((b) => b.trim()).filter((b) => b.length > 10);
...
*/
async function aiExtractQuestions(rawText, apiKey) {
	...
	for (const batch of batches) {
		const result = await extractBatch(batch.join("\n\n"), apiKey);
...
async function extractBatch(text, apiKey) {
	...
	if (!parsed) throw new Error("AI returned invalid JSON — could not parse question data");
	return {
